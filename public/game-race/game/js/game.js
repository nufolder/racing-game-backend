...
//powers
var scoreData = {
    text: "SCORE: [NUMBER]", //score display text
    coin: 1
}; //collect coin score
var coinData = {
...
var nitroData = {
    maxSpeed: 20000, //nitro max speed
            ...
            TweenMax.to(playerData, 1, {
                ...
                onUpdate: function() {
                    ...
                    resultCoinTxt.text =
                        ...
                        " KOIN BISA DIGUNAKAN UNTUK UNLOCK RIDER";
                }
            });
...
function startGame() {
    ...
    keyData.accelerate = true;
    updateGameText(
        statusData.start.text,
        statusData.start.color,
        statusData.start.size,
        0
    );
    updateGameStatus();
}
...
function updateGame() {
    ...
    if (!gameData.paused) {
        ...
        const dataCurrent = score_value_data / 100;
        playerData.score += Math.floor(
            dataCurrent *
                Math.round(defaultData.speed / score_value_data) *
                0.03
        );
        updateGameStatus();
    }
...
function updatePenaltyTimer() {
    ...
    if (
        String(gameData.penaltyTime * 0.1).indexOf(".") == -1 &&
        displayPenaltyTimer
    ) {
        playSound("soundTick");
    }
    ...
    if (gameData.penaltyTime <= 0) {
        updateGameText(
            statusData.penalty.text.replace("[NUMBER]", "0.00"),
            statusData.penalty.color,
            statusData.penalty.size,
            0
        );
        ...
        if (displayPenaltyTimer) {
            var curPenaltyTime = (
                String(gameData.penaltyTime * 0.1) + "000"
            ).substring(0, 4);
            updateGameText(
                statusData.penalty.text.replace("[NUMBER]", curPenaltyTime),
                statusData.penalty.color,
                statusData.penalty.size,
                0
            );
...
function updateSprites() {
    ...
    for (n = 0; n < playerSegment.cars.length; n++) {
        ...
        if (defaultData.speed > car.speed) {
            if (
                getOverlap(defaultData.playerX, playerW, car.offset, carW, 0.8)
            ) {
...
function updateCars(dt, playerSegment, playerW) {
    ...
    for (n = 0; n < cars.length; n++) {
        ...
        car.offset =
            car.offset +
            updateCarOffset(car, oldSegment, playerSegment, playerW);
...
function updateCarOffset(car, carSegment, playerSegment, playerW) {
    ...
    for (i = 1; i < lookahead; i++) {
        ...
        if (
            ...
            car.speed > defaultData.speed &&
            getOverlap(defaultData.playerX, playerW, car.offset, carW, 1.2)
        ) {
            if (defaultData.playerX > 0.5) dir = -1;
            else if (defaultData.playerX < -0.5) dir = 1;
            else dir = car.offset > defaultData.playerX ? 1 : -1;
            return (
                (((dir * 1) / i) * (car.speed - defaultData.speed)) /
                worldData.maxSpeed
            );
        }
...
function renderWorld() {
    ...
    for (n = 0; n < defaultData.drawDistance; n++) {
        ...
        getProject(
            segment.p1,
            defaultData.playerX * roadData.width - x,
            playerY + worldData.cameraHeight,
            defaultData.position -
                (segment.looped ? defaultData.trackLength : 0),
        ...
        getProject(
            segment.p2,
            defaultData.playerX * roadData.width - x - dx,
            playerY + worldData.cameraHeight,
            defaultData.position -
                (segment.looped ? defaultData.trackLength : 0),
            defaultData.cameraDepth,
    ...
    for (n = defaultData.drawDistance - 1; n > 0; n--) {
        ...
        if (segment == playerSegment) {
            renderPlayer(
                ...
                roadData.width,
                sprites,
                defaultData.speed / worldData.maxSpeed,
...
function findSegment(z) {
    return segments[
        Math.floor(z / defaultData.segmentLength) % segments.length
    ];
}
...
function addSegment(curve, y) {
    ...
    segments.push({
        ...
        cars: [],
        color:
            Math.floor(n / roadData.rumbleLength) % 2
                ? roadData.dark
                : roadData.light
    });
}
...
function addRoadType(type, num, height, curve) {
    switch (type) {
        ...
        case "sCurves":
            addRoad(
                roadLengthData.length.medium,
                roadLengthData.length.medium,
                roadLengthData.length.medium,
                -roadLengthData.curve.easy,
                roadLengthData.hill.none
            );
            addRoad(
                roadLengthData.length.medium,
                roadLengthData.length.medium,
                roadLengthData.length.medium,
                roadLengthData.curve.medium,
                roadLengthData.hill.medium
            );
            addRoad(
                roadLengthData.length.medium,
                roadLengthData.length.medium,
        ...
        case "end":
            ...
            addRoad(
                ...
                num,
                -roadLengthData.curve.easy,
                -getLastY() / defaultData.segmentLength
            );
            break;
    }
}
...
function resetSprites() {
    ...
    for (var n = 10; n < segments.length; n += 10) {
        addSprite(
            n,
            randomChoice(spritesData.PLANTS),
            randomChoice([1, -1]) * (1.2 + Math.random() * 5)
        );
    }
    ...
    if (spritesData.BILLBOARDS.length > 0) {
        for (
            var n = 100;
            n < segments.length;
            n += 300 + Math.floor(Math.random() * 100)
        ) {
            addSprite(
                n,
                randomChoice(spritesData.BILLBOARDS),
...
function resetCollectItems() {
    for (var n = 0; n < segments.length; n++) {
        ...
        for (var s = 0; s < curSegment.sprites.length; s++) {
            ...
            if (
                sprite.source.id == "NITRO" ||
                sprite.source.id == "COIN" ||
    ...
    //coin
    for (
        var n = randomInt(20, 50);
        n < segments.length;
        n += 300 + Math.floor(Math.random() * 50)
    ) {
        addSprite(
            n,
            spritesData.COIN,
            randomChoice([1, -1]) * (0.1 + Math.random() * 0.3)
        );
    }
}
...
function resetCars() {
    ...
    for (var n = 0; n < defaultData.totalCars; n++) {
        ...
        speed =
            worldData.maxSpeed / 4 +
            (Math.random() * worldData.maxSpeed) /
                (sprite == spritesData.SEMI ? 4 : 2);
...
function renderSprite(
    ...
    var destW =
        ((newSprite.image.naturalWidth * scale * width) / 2) *
        (defaultData.scale * roadWidth);
    var destH =
        ((newSprite.image.naturalHeight * scale * width) / 2) *
        (defaultData.scale * roadWidth);
...
function renderPlayer(
    ...
    if (steer < 0)
        ...
        sprite =
            updown > 0 ? playerCarData.up_straight : playerCarData.straight;
...
function renderCar(
    ...
    var newSprite = $.sprites[sprite.id].clone();
    var destW =
        ((sprite.w * scale * width) / 2) * (defaultData.scale * roadWidth);
    var destH =
        ((sprite.h * scale * width) / 2) * (defaultData.scale * roadWidth);
    ...
    if (clipH < destH) {
        ...
        } else if (defaultData.playerX < -0.9) {
            leftSmoke = true;
...
function startNitro() {
    if (!gameData.nitroMode) {
        ...
        updateGameText(
            statusData.nitro.text,
            statusData.nitro.color,
            statusData.nitro.size,
            1
        );
...
function addScore() {
    ...
    updateGameText(
        statusData.score.text.replace("[NUMBER]", scoreData.coin),
        statusData.score.color,
        statusData.score.size,
        1
    );
}
...
function addCoin() {
    ...
    updateGameText(
        statusData.coin.text.replace("[NUMBER]", coinData.coin),
        statusData.coin.color,
        statusData.coin.size,
        1
    );
}
...
function addFuel() {
    ...
    gameData.fuel =
        gameData.fuel > fuelData.total ? fuelData.total : gameData.fuel;
    updateGameText(
        statusData.fuel.text,
        statusData.fuel.color,
        statusData.fuel.size,
        1
    );
}
...
function updateGameStatus() {
    //score
    scoreTxt.text = scoreShadowTxt.text = scoreData.text.replace(
        "[NUMBER]",
        addCommas(playerData.score)
    );
    coinTxt.text = coinShadowTxt.text = coinData.text.replace(
        "[NUMBER]",
        addCommas(playerData.coin)
    );
    lifeTxt.text = lifeData.text.replace("[NUMBER]", addCommas(lifeData.life));

    //fuel
    var fuelPercent =
        (gameData.fuel / fuelData.total) * fuelData.bar.width -
        fuelData.bar.space * 2;
    fuelPercent = fuelPercent < 0 ? 0 : fuelPercent;
    fuelBarFill.graphics.clear();
    fuelBarFill.graphics
        .beginFill(fuelData.bar.fillColor)
        .drawRect(
            0,
            0,
            fuelPercent,
            fuelData.bar.height - fuelData.bar.space * 4
        );

    if (gameData.fuel < (fuelData.total / 100) * 25 && !gameData.penalty) {
        updateGameText(
            statusData.lowFuel.text,
            statusData.lowFuel.color,
            statusData.lowFuel.size,
            0
        );
    }
    ...
    if (!gameData.paused && gameData.fuel <= 0) {
        updateGameText(
            statusData.noFuel.text,
            statusData.noFuel.color,
            statusData.noFuel.size,
            0
        );
        endGame();
    }
...
function updateGameText(text, color, size, delay) {
    ...
    if (delay > 0) {
        TweenMax.to(gameStatusContainer, delay, {
            overwrite: true,
            onComplete: function() {
