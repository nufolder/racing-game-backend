<!-- ttc -->
<div class="modal fade select-rider" id="ttc_atc" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog  modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body p-0">
                <button type="button" class="ico-close" data-bs-dismiss="modal" aria-label="Close"><img
                        src="{{ asset('images/ico-close.png') }}"></button>
                <div id="carousel-ttc_atc">

                    <div>
                        <img src="{{ asset('photo-riders/rider-chessy.jpg') }}" class="card-img-top"
                            alt="Chessy Meilandri">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Chessy Meilandri</div>
                                <div class="age">12 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/chessy') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - Astra Honda Racing School</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-1.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <img src="{{ asset('photo-riders/rider-veda.jpg') }}" class="card-img-top"
                            alt="Veda Ega Pratama">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Veda Ega Pratama</div>
                                <div class="age">13 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/veda') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - Astra Honda Racing School</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-1.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <img src="{{ asset('photo-riders/rider-fadillah.jpg') }}" class="card-img-top"
                            alt="Chessy Meilandri">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Fadillah arbi aditama</div>
                                <div class="age">16 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/fadillah') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - Astra Honda Racing School</li>
                                        <li>2018 - Astra Honda Racing School</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-2.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <img src="{{ asset('photo-riders/rider-azryan.jpg') }}" class="card-img-top"
                            alt="Veda Ega Pratama">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Azryan Dheyo W.</div>
                                <div class="age">16 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/azryan') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - 3rd Overall Standing Point Thailand Talent Cup</li>
                                        <li>2018 - Astra Honda Racing School</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-2.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <img src="{{ asset('photo-riders/rider-herlian.jpg') }}" class="card-img-top"
                            alt="Veda Ega Pratama">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Herlian Dandi</div>
                                <div class="age">15 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/herlian') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - 5th Overall Standing Point Thailand Talent Cup</li>
                                        <li>2018 - Astra Honda Racing School</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-2.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <img src="{{ asset('photo-riders/rider-herjun-atc.jpg') }}" class="card-img-top"
                            alt="Veda Ega Pratama">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Herjun Atna Firdaus</div>
                                <div class="age">17 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/herjun') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - 9th Position Overall Standing Asia Talent Cup</li>
                                        <li>2018 - Champion of Thailand Talent Cup</li>
                                        <li>2017 - Astra Honda Racing School</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-2.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </div>
</div>



<!-- arrc AP250-->
<div class="modal fade select-rider" id="arrc_ap250" tabindex="-1" aria-labelledby="exampleModalLabel2"
    aria-hidden="true">
    <div class="modal-dialog  modal-dialog-centered ">
        <div class="modal-content">
            <div class="modal-body p-0">
                <button type="button" class="ico-close" data-bs-dismiss="modal" aria-label="Close"><img
                        src="{{ asset('images/ico-close.png') }}"></button>
                <div id="carousel-arrc_ap250">

                    <div>
                        <img src="{{ asset('photo-riders/rider-herjun-arrc.jpg') }}" class="card-img-top" alt="Chessy Meilandri">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Herjun atna firdaus</div>
                                <div class="age">17 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/herjun-arrc') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - 9th Position Overall Standing Asia Talent Cup</li>
                                        <li>2018 - Champion of Thailand Talent Cup</li>
                                        <li>2018 - Astra Honda Racing School CBR150R</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-3.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <img src="{{ asset('photo-riders/rider-adenanta.jpg') }}" class="card-img-top">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">M. adenanta putra</div>
                                <div class="age">17 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/adenanta') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - 4th position Overall Standing Asia Talent Cup</li>
                                        <li>2018 - 11th Position Overall Standing Asia Talent Cup 2018</li>
                                        <li>2018 - 16th position on Overall Standing IRS SP250</li>
                                        <li>2017 - 1st Position Race 1 Rd. 3 Thailand Talent Cup</li>
                                        <li>2017 - 1st Position Race 1 Rd. 4 Thailand Talent Cup</li>
                                        <li>2017 - 2nd Position Race 2 Rd. 3 Thailand Talent Cup</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-3.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <img src="{{ asset('photo-riders/rider-lucky.jpg') }}" class="card-img-top">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Lucky Hendriansya</div>
                                <div class="age">20 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/lucky') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - 5th position Overall Standing Asia Road Racing Championship AP250
                                        </li>
                                        <li>2018 - 8th position Overall Standing Asia Talent Cup 2018</li>
                                        <li>2017 - 10th position Overall Standing Asia Talent Cup 2017</li>
                                        <li>2016 - Astra Honda Racing School 250cc 2016</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-3.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </div>
</div>


<!-- arrc -->
<div class="modal fade select-rider" id="arrc_ss600" tabindex="-1" aria-labelledby="exampleModalLabel2"
    aria-hidden="true">
    <div class="modal-dialog  modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body p-0">
                <button type="button" class="ico-close" data-bs-dismiss="modal" aria-label="Close"><img
                        src="{{ asset('images/ico-close.png') }}"></button>
                <div id="carousel-arrc_ss600">

                    <div>
                        <img src="{{ asset('photo-riders/rider-rheza.jpg') }}" class="card-img-top">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Rheza Danica Ahrens</div>
                                <div class="age"><span>The Silent Boy</span> - 23 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/rheza') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - 11th Position Overall Standing of Champion of Asia Road Racing</li>
                                        <li>2019 - Championship SS600 class</li>
                                        <li>2018 - Champion of Asia Road Racing Championship AP250 class</li>
                                        <li>2017 - 4th Position on Overall Standing ARRC 2017 AP250</li>
                                        <li>2017 - 1st Position on Overall Standing IRS 2017 SS600</li>
                                        <li>2017 - 7th Position Suzuka 4 Hours Endurance Race 2017</li>
                                        <li>2016 - 1st position Suzuka 4 Hours Endurance Race</li>
                                        <li>2016 - 4th position Indospeed Race Series SS600 2016</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-3.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <img src="{{ asset('photo-riders/rider-irfan.jpg') }}" class="card-img-top">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Irfan Ardiansyah</div>
                                <div class="age"><span>Irfantastic</span> - 22 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/irfan') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2019 - 3rd position Overal Standing Asia Road Racing Championship AP250
                                            class</li>
                                        <li>2018 - 15th position Overall Standing Asia Road Racing Championship SS600
                                            class</li>
                                        <li>2017 - 15th position Overall Standing Asia Road Racing Championship SS600
                                            class</li>
                                        <li>2016 - 1st position Suzuka 4 Hours Endurance Race</li>
                                        <li>2016 - 3rd position Indospeed Race Series SS-600 2016</li>
                                        <li>2015 - 1st position Astra Honda Racing School 250</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-3.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </div>
</div>


<!-- cev -->
<div class="modal fade select-rider" id="cev" tabindex="-1" aria-labelledby="exampleModalLabel3" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body">
                <button type="button" class="ico-close" data-bs-dismiss="modal" aria-label="Close"><img
                        src="{{ asset('images/ico-close.png') }}"></button>
                <div id="carousel-cev">
                    <div>
                        <img src="{{ asset('photo-riders/rider-mario.jpg') }}" class="card-img-top">
                        <div class="card-body">
                            <div class="card-details ">
                                <div class="name">Mario Suryo Aji</div>
                                <div class="age"><span>Super Mario</span> - 17 Tahun</div>
                                <div class="pt-3 px-5"><a href="{{ url('race/mario') }}"
                                        class="btn btn-sm btn-primary">GO!!</a></div>
                                <div class="achievement">
                                    <ul>
                                        <li>2020 - 16th Position Overall Standing CEV Moto 3 Junior World Championship
                                        </li>
                                        <li>2020 - 11th Position Overall Standing Red Bull Rookies Cup</li>
                                        <li>2019 - 18th Position Overall Standing CEV Moto 3 Junior World Championship
                                        </li>
                                        <li>2019 - 16th Position Overall Standing Red Bull Rookies Cup</li>
                                        <li>2018 - 5th Position Overall Standing Asia Talent Cup</li>
                                        <li>2018 - 6th Position Overall Standing ARRC AP250</li>
                                        <li>2017 - 3rd Position Race 1 Rd. 5 Thailand Talent Cup</li>
                                        <li>2017 - 2nd Position Race 2 Rd. 5 Thailand Talent Cup</li>
                                    </ul>
                                </div>
                                <div class="benefit">Power* <img src="{{ asset('images/level-4.jpg') }}"></div>
                                <div class="benefit-details">Power berfungsi untuk mendapatkan skor lebih besar pada
                                    setiap game dan akan berpengaruh pada perolehan jumlah tiket yang didapat.</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>